...
8c. **Zzz 要大要粗、不要漸層淡出**：睡覺的 z / zz / zzz 一律**大、粗體、實心深色**，清楚可讀。
    **不可用「漸淡/半透明」的 Zzz**——淡的字對比低會被去背當背景清掉。
    ...
    ★ 兩個節奏錯開：**肚子起伏 2 格一循環**(消/鼓交替)、**Zzz 3 格一循環**(z/zz/zzz)，
    6 格剛好是最小公倍數，每格組合都不同，才不會重複單調。
    ★ **肚子鼓起時只改「外型輪廓」，填色與陰影保持不變**（同一塊色、同一個深淺），
    不要把鼓起的肚子畫成不同顏色或多加高光/陰影，否則動起來顏色會閃。
...
## ★ 開羅模式（Kairosoft 式，新動物一律用這個）

參考開羅遊戲的做法：**小像素角色＋極少格數，動態交給程式**。效率高、AI 生圖穩定。

- **每隻動物只要一張 1×9 條**：`assets/animal_<id>.png`（cell 沿用畫素表）
  格序：`前走1 前走2 | 側走1 側走2(朝左) | 背走1 背走2 | 坐 | 睡 | 吃(低頭)`
- **不畫**：陰影、Zzz、食物、左右向——全部程式處理：
  - 右向＝程式把側面翻轉(flipX)；走路彈跳(bob)程式加
  - 睡覺 Zzz＝程式疊加的文字特效(z/zz/zzz循環飄浮)
  - 進食＝程式在嘴前疊加肉塊、最後2秒換骨頭
  - 陰影＝程式橢圓（`bakedShadow` 不設）
- `config.js` 該動物設 `kairo: true` → render.js 走開羅分支（walk 2格交替、坐6/睡7/吃8 靜態格）
- **像素風**：pixelArt 已開(取樣銳利)。生圖用 pixel art 風格、純白背景、無抗鋸齒
  → 去背用 `cutout.py --bg FFFFFF`(flood-fill 對像素圖是完美的)，不需 rembg。
- 獅子維持舊制(精緻模式多表)；其餘動物用開羅模式。
