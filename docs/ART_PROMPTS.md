...
**★ 單格像素依「動物體型」調整（以獅子 64×64 為基準）**

不同動物天生大小不同，**單格(frame)像素**要照體型縮放（遊戲會依此載入並顯示）：

| 動物 | 單格 frame | 走路整張(frame×4) | 進食/睡覺(frame×4 寬 × frame 高) |
|---|---|---|---|
| 獅子 lion | 64 | 256×256 | 256×64 |
| 猴子 monkey | 56 | 224×224 | 224×56 |
| 企鵝 penguin | 48 | 192×192 | 192×48 |
| 大象 elephant | 96 | 384×384 | 384×96 |
| 長頸鹿 giraffe | 96 | 384×384 | 384×96 |

> 動物要**畫滿整格、腳貼格子底部**；格子大小不同就是體型差異的來源。
> 若 AI 不好控制像素，生大張(1024)再用 `cutout.py --resize` 縮到上表尺寸即可
> （例：大象走路 `--resize 384x384`、企鵝進食 `--resize 192x48`）。
